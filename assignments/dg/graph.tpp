...
operator=(const gdwg::Graph<N, E> &g) noexcept {
  this->nodes_.clear();
  for (const auto &node : g.nodes_) {
    this->nodes_.insert(node);
  }
...
template <typename N, typename E>
bool gdwg::Graph<N, E>::Node::AddEdge(std::shared_ptr<Node> src,
                                      std::shared_ptr<Node> dst, const E &w) {
  auto it =
      std::find_if(src->edges_.begin(), src->edges_.end(),
                   [&src, &dst, &w](std::shared_ptr<Edge> e) {
                     return (src == e->GetSource().lock() &&
                             dst == e->GetDest().lock() && w == e->GetValue());
                   });
  if (!(it == src->edges_.end()))
    return false;
...
void gdwg::Graph<N, E>::MergeReplace(const N &oldData, const N &newData) {
  ...
  for (const auto &node : nodes_) {
    for (const auto &edge : node.second->GetEdges()) {
      if ((edge->GetSource().lock())->GetValue() == oldData) {
        InsertEdge(newData, (edge->GetDest().lock())->GetValue(),
                   edge->GetValue());
        nodes_[newData]->GetEdges().erase(edge);
      } else if ((edge->GetDest().lock())->GetValue() == oldData) {
        InsertEdge((edge->GetSource().lock())->GetValue(), newData,
                   edge->GetValue());
...
template <typename N, typename E> void gdwg::Graph<N, E>::Clear() noexcept {
  for (const auto &node : nodes_) {
...
gdwg::Graph<N, E>::find(const N &src, const N &dst, const E &w) noexcept {
  return std::find_if(
      begin(), end(), [&src, &dst, &w](const std::tuple<N, N, E> curr) {
        return (std::get<0>(curr) == src && std::get<1>(curr) == dst &&
                std::get<2>(curr) == w);
      });
}
...
gdwg::Graph<N, E>::erase(gdwg::Graph<N, E>::const_iterator it) noexcept {
  auto res =
      std::find_if(begin(), end(), [&it](auto curr) { return *it == curr; });
  if (res == end()) {
    return res;
  }
  res++;
...
operator++() {
  ...
  if (edge_it_ == node_it_->second->GetEdges().end()) {
    do {
      ++node_it_;
    } while (node_it_ != sentinel_ && node_it_->second->GetEdges().begin() ==
                                          node_it_->second->GetEdges().end());
...
typename gdwg::Graph<N, E>::const_iterator &gdwg::Graph<N, E>::const_iterator::
operator--() {
  // decrementing from end() on bi-directional iterator should return last
  // element
  if (node_it_ == sentinel_) {
    node_it_ = last_node_;
  }
  ...
  // If edge iterator points to first edge of a node, it wraps around to the
  // last edge of the previous node
  if (edge_it_ == node_it_->second->GetEdges().begin()) {
    node_it_ = std::prev(node_it_);

    // Traverse to the first node that contains an edge, update iterator as you
    // go
    while (node_it_->second->GetEdges().size() == 0) {
      node_it_ = std::prev(node_it_);
      edge_it_ = node_it_->second->GetEdges().end();
...
typename gdwg::Graph<N, E>::const_iterator gdwg::Graph<N, E>::cbegin() {
  return {nodes_.begin(), nodes_.end(), std::prev(nodes_.end()),
          nodes_.begin()->second->GetEdges().begin()};
}

template <typename N, typename E>
typename gdwg::Graph<N, E>::const_iterator gdwg::Graph<N, E>::cend() {
  return {nodes_.end(), nodes_.end(), std::prev(nodes_.end()),
          nodes_.begin()->second->GetEdges().end()};
}
