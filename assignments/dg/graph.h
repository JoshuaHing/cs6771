...
class Graph {
 public:
  ...
  class const_iterator {
   ...
   private:
    // Outer iterator for nodes
    typename std::map<N, std::shared_ptr<Node>>::iterator node_it_;

    // Keep track of end() for nodes
    const typename std::map<N, std::shared_ptr<Node>>::iterator sentinel_;

    // Keep track of the value before end() for compatibility with reverse_iterator
    typename std::map<N, std::shared_ptr<Node>>::iterator last_node_;

    // Inner iterator for edges
