...
struct CompareByValue {
  bool operator()(const std::shared_ptr<T> &lhs,
    ...
    if (lhs->GetSource().lock()->GetValue() <
        rhs->GetSource().lock()->GetValue()) {
      return true;
    }
    ...
    if (lhs->GetSource().lock()->GetValue() <=
            rhs->GetSource().lock()->GetValue() &&
        lhs->GetDest().lock()->GetValue() <=
            rhs->GetDest().lock()->GetValue() &&
...
public:
  ...
  public:
    ...
    // Getters / Setters
    std::set<std::shared_ptr<Edge>, CompareByValue<Edge>> &GetEdges() {
      return edges_;
    }
    N &GetValue() { return value_; }
    void SetValue(const N &val) { value_ = val; }
    void SetEdges(std::set<std::shared_ptr<Edge>, CompareByValue<Edge>> e) {
      edges_ = e;
    }

    // Helper methods
    bool AddEdge(std::shared_ptr<Node> src, std::shared_ptr<Node> dst,
                 const E &w);
  ...
  private:
    ...
    // Inner edge iterator
    typename std::set<std::shared_ptr<Edge>, CompareByValue<Edge>>::iterator
        edge_it_;
    ...
    const_iterator(const decltype(node_it_) &node_it,
        ...
        : node_it_{node_it}, sentinel_{sentinel},
  ...
  Graph(typename std::vector<std::tuple<N, N, E>>::const_iterator tup_begin,
  ...
  friend std::ostream &operator<<(std::ostream &os,
    ...
    for (const auto &node : g.nodes_) {
      ...
      for (const auto &ptr_set : node.second->GetEdges()) {
        os << "  " << ptr_set->GetDest().lock()->GetValue() << " | "
           << ptr_set->GetValue() << "\n";
      }
      os << ")\n";
    }
